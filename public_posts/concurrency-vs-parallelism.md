```
Title:   Concurrency-and-Parallelism,GIL
...
首先需要明确的一点是GIL并不是Python的特性，它是在实现Python解析器(CPython)时所引入的一个概念。JPython就没有GIL.
> 就好比C++是一套语言（语法）标准，但是可以用不同的编译器来编译成可执行代码。有名的编译器例如GCC，INTEL C++，Visual C++等。Python也一样，同样一段代码可以通过CPython，PyPy，Psyco等不同的Python执行环境来执行。像其中的JPython就没有GIL。然而因为CPython是大部分环境下默认的Python执行环境。所以在很多人的概念里CPython就是Python，也就想当然的把GIL归结为Python语言的缺陷。所以这里要先明确一点：GIL并不是Python的特性，Python完全可以不依赖于GIL。

### CPython实现中的GIL是什么
GIL全称Global Interpreter Lock. 全局解释器Lock.
官方给出的解释：
...
`即同一时刻只有一个线程使用CPU运行`，也就是说多线程并不是真正意义上的同时执行.

As GIL is a Mutex(互斥锁), which mean 只有一个thread可以获取GIL，在一个时间点。 
...
In Multi-core CPU, a process with multi-threads, threads can (并行)parallilism run.
...
### GIL effect
  If So, python can not benifit from multi-core-CPU.
  Under GIL, only one thread(on a cpu) can run.

### GIL的影响

从上文的介绍和官方的定义来看，GIL无疑就是一把全局互斥锁。毫无疑问全局锁的存在会对多线程的效率有不小影响。甚至就几乎等于Python是个单线程的程序。
...
![python-gil-2-compute-tense-threads-performance.jpg](./pics/concurrency-vs-parallelism/python-gil-2-compute-tense-threads-performance.jpg)
...
![python-gil-io-tense-threads-performance.jpg](./pics/concurrency-vs-parallelism/python-gil-io-tense-threads-performance.jpg)
